...
async function serveFile(res, pathname) {
  ...
  try {
    ...
    res.writeHead(200, { 'Content-Type': `${mime}; charset=utf-8`, 'Cache-Control': pathname.startsWith('/assets/') ? 'public, max-age=31536000, immutable' : 'no-cache' });
    res.end(data);
  } catch {
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/auth/start' && req.method === 'GET') {
      ...
      redirect(res, target.toString(), { 'Set-Cookie': cookie('ks_oauth', sign({ state, verifier, roomId, exp: Date.now() + 10 * 60_000 }), 600) });
      return;
    }
    if (pathname === '/auth/callback' && req.method === 'GET') {
      ...
      redirect(res, `${process.env.PUBLIC_ORIGIN}/?multi=1${oauth.roomId ? `&room=${oauth.roomId}` : ''}`, { 'Set-Cookie': [
    ...
    if (pathname.startsWith('/api/rooms')) {
      ...
      if (action === 'start') startRoom(room, user);
        ...
        await addMessage(room, user, text);
    ...
    fail(res, error, error.message?.startsWith('Database request failed') ? 503 : 400);
  }
});
