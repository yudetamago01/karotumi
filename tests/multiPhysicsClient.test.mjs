...
test('a drop is visible immediately and worker frames keep it moving', () => {
  ...
  try {
    ...
    worker.onmessage({ data: { poses: [[predicted.id, predicted.x, predicted.y + 12, 0, 0, 2, 0]] } });
    assert.ok(client.pose(predicted.id).y > predicted.y, 'simulation frames advance the fall');
    worker.onmessage({ data: { poses: [[predicted.id, predicted.x, predicted.y + 28, 0, 0, 2, 0]] } });
    assert.ok(client.pose(predicted.id).y > predicted.y + 12);
    client.dispose();
